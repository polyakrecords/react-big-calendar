...
var DaySlot = _react2.default.createClass({
  ...
  positionTimeIndicator: function positionTimeIndicator() {
    ...
    var daysPassed = _dates2.default.diff(start, now, 'day');

    var timeIndicator = this.refs.timeIndicator;
    var factor = secondsPassed / secondsGrid;

    if (now >= start && now <= end && _dates2.default.eq(date, now, 'day')) {
      // const pixelHeight = timeGutter.offsetHeight;
      var offset = Math.floor(100 * factor);
  ...
  triggerTimeIndicatorUpdate: function triggerTimeIndicatorUpdate() {
    var _this = this;
    ...
    this._timeIndicatorTimeout = window.setTimeout(function () {
      ...
      _this.triggerTimeIndicatorUpdate();
    }, 60000);
  },
  render: function render() {
    ...
    var timeslots = _props2.timeslots;
    var now = _props2.now;
    var selectRangeFormat = _props2.selectRangeFormat;
    var culture = _props2.culture;

    var props = _objectWithoutProperties(_props2, ['min', 'max', 'step', 'timeslots', 'now', 'selectRangeFormat', 'culture']);
  ...
  renderEvents: function renderEvents() {
    ...
    var events = _props3.events;
    var step = _props3.step;
    var min = _props3.min;
    var culture = _props3.culture;
    var eventPropGetter = _props3.eventPropGetter;
    var selected = _props3.selected;
    var eventTimeRangeFormat = _props3.eventTimeRangeFormat;
    var eventComponent = _props3.eventComponent;
    var startAccessor = _props3.startAccessor;
    var endAccessor = _props3.endAccessor;
    var titleAccessor = _props3.titleAccessor;
  ...
  _selectable: function _selectable() {
    ...
    selector.on('click', function (_ref4) {
      ...
      _this3._clickTimer = setTimeout(function () {
        _this3._selectSlot(selectionState({ x: x, y: y }));
      });

      _this3.setState({ selecting: false });
    });
